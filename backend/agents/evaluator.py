...
def _ungrounded_urls(evidence_urls: list[str], retrieved: set[str]) -> list[str]:
    """返回「不在检索集（精确或域名）」里的证据 URL。

    这些 URL 疑似模型未据搜证结果、而是凭记忆自填——事实核查器最该警惕的失败。
    当前只观测（日志告警），不丢弃：实测幻觉率为 0%，留观测口待真有数据再决定是否拦截。
    """
    domains = {_domain(u) for u in retrieved}
    return [
        u
        for u in evidence_urls
        if u not in retrieved and _domain(u) not in domains
    ]
...
class ClaudeEvaluatorAgent:
    ...
    async def evaluate(self, claim: Claim) -> Evaluation:
        gathered = await self._gather(claim)
        if not gathered:
            return self._unverifiable(claim, "证据检索失败或无可靠来源，暂判为无法核实。")
        research, retrieved = gathered
        ...
        # 溯源观测：证据 URL 不在真实检索集里 → 疑似模型自填，告警但不丢弃。
        for url in _ungrounded_urls([e.source_url for e in evidence], retrieved):
            logger.warning(
                "[factcheck] 证据 URL 不在检索集（疑似未据搜证自填） claim=%s url=%s",
                claim.id,
                url,
            )
        return Evaluation(
    ...
    async def _gather(self, claim: Claim) -> Optional[tuple[str, set[str]]]:
        """第一段：联网搜证。

        返回 (发现摘要文本, 真实检索到的 URL 集合)；搜证失败 / 无正文返回 None。
        URL 集合取自 web_search_tool_result 块（API 返回的原始检索结果，是溯源 ground truth），
        供 evaluate 校验证据是否真有据可依。
        """
        try:
