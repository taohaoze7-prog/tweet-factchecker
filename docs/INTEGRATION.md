...
- `CLAIM_TIMEOUT_S`：初设 120s 太紧，真实两段式联网 evaluator（3 次串行 sonnet）会被误降级；
  现取 **150s**（`core.py` 实际值）——冷启动实测 ~103s，留 ~47s 余量，超时只兜病态卡死。
  > ⚠️ 该余量在高负载/慢搜证下偏紧，数值是否上调待 ③ URL 校验采样真链路时一并复核。
- 验证：新增「evaluator 抛错→整条降级不崩」单测；真链路恢复 `true / 0.99`。

### 结论透明化（覆盖率）

- `core.aggregate`：整体判定只基于「已核实」断言（`UNVERIFIABLE` 不计入数值聚合），
  但 `summary` 现明说覆盖率——「共 N 条，M 条已核实…另有 K 条无法核实」，
  杜绝「3 条只核实 1 条也被头部 verdict 一笔带过」的误导。`summary` 为自由文本，不动契约。
- 验证：新增混合覆盖 / 全不可核实两条单测。
